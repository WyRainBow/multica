...
| Interactive non-done → `done` blocks on unreviewed comment threads; resolve only necessary discussions and explicitly keep the rest; keep receipts are snapshot-bound and resolution points at a chosen comment | `server/internal/handler/issue_done_comment_review.go`; `UpdateIssue` / `BatchUpdateIssues` integration in `server/internal/handler/issue.go` | COC-236 |
| CLI retry contract (`--comment-review-file`) | `server/cmd/multica/cmd_issue_done_review.go`; flag registration and status request in `server/cmd/multica/cmd_issue.go` | COC-236 |
