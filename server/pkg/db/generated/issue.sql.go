...
const archiveIssueSubtree = `-- name: ArchiveIssueSubtree :many
WITH RECURSIVE subtree AS (
    SELECT root.id
    FROM issue root
    WHERE root.id = $2 AND root.workspace_id = $3
    UNION
    SELECT child.id
    FROM issue child
...
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, archived_at, archived_by, parked_from_issue_id, status_changed_at, description_revision, created_by_session
`

type ArchiveIssueSubtreeParams struct {
...
// terminates.
func (q *Queries) ArchiveIssueSubtree(ctx context.Context, arg ArchiveIssueSubtreeParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, archiveIssueSubtree, arg.ArchivedBy, arg.ID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, archived_at, archived_by, parked_from_issue_id, status_changed_at, description_revision, created_by_session FROM issue
WHERE workspace_id = $1
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.archived_at, i.archived_by, i.parked_from_issue_id, i.status_changed_at, i.description_revision, i.created_by_session FROM issue i
WHERE i.workspace_id = $1
...
const listParkedFromIssue = `-- name: ListParkedFromIssue :many
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, archived_at, archived_by, parked_from_issue_id, status_changed_at, description_revision, created_by_session FROM issue
WHERE workspace_id = $1
  AND parked_from_issue_id = $2
  AND archived_at IS NULL
ORDER BY created_at ASC
`

type ListParkedFromIssueParams struct {
	WorkspaceID       pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) ListParkedFromIssue(ctx context.Context, arg ListParkedFromIssueParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listParkedFromIssue, arg.WorkspaceID, arg.ParkedFromIssueID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const parkIssue = `-- name: ParkIssue :one
UPDATE issue
SET parent_issue_id      = NULL,
    stage                = NULL,
    status               = 'backlog',
    parked_from_issue_id = $1,
    updated_at           = now()
WHERE id = $2 AND workspace_id = $3
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, archived_at, archived_by, parked_from_issue_id, status_changed_at, description_revision, created_by_session
`
...
func (q *Queries) ParkIssue(ctx context.Context, arg ParkIssueParams) (Issue, error) {
	row := q.db.QueryRow(ctx, parkIssue, arg.ParkedFromIssueID, arg.ID, arg.WorkspaceID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const unarchiveIssueSubtree = `-- name: UnarchiveIssueSubtree :many
WITH RECURSIVE subtree AS (
    SELECT root.id
    FROM issue root
    WHERE root.id = $1 AND root.workspace_id = $2
    UNION
    SELECT child.id
    FROM issue child
    JOIN subtree ON child.parent_issue_id = subtree.id
...
type UnarchiveIssueSubtreeParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}

// Mirror of ArchiveIssueSubtree. Restores the issue and its descendants.
func (q *Queries) UnarchiveIssueSubtree(ctx context.Context, arg UnarchiveIssueSubtreeParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, unarchiveIssueSubtree, arg.ID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
UPDATE issue SET
    ...
    status_changed_at = CASE
        WHEN $4::text IS NOT NULL
         AND $4::text IS DISTINCT FROM status
    ...
    description_revision = CASE
        WHEN $3::text IS NOT NULL
         AND $3::text IS DISTINCT FROM description
