...
- **一键复制正文**，拿到的是 Markdown 原文 —— 跟 `multica issue get` 返回的、Agent 读进去的是同一份。复制渲染后的文字会让标题、列表、代码块统统变成普通行。已完成 / 已取消的 issue 上也有：复制不是编辑，而冻结的正文恰恰最常被搬去别处
...
- **点一条跳到正文对应位置**，落点上方留一点余白 —— 贴着顶边会让人以为这一节的开头还在屏幕外
- **已完成 / 已取消的 issue 也有目录**。正文冻结后走的是只读渲染、不加载编辑器，原来这类 issue 一条目录都没有 —— 而恰恰是这些正文最常被回头翻
- 文档页用的是同一套目录
...
- 编辑**自动存**，没有保存按钮
- **哪里都显示字数**：列表行、文档页底部、issue 上的关联列表，以及 `wiki list` 的 CHARS 列。列表的正文预览是按固定高度切的，一份 300 字的便条和一份 11674 字的 SOP 在屏幕上一样高，字数是唯一说明「切口下面还有多少」的东西。四处都按**码点**数（`🎉` 算 1 个），所以四个数字能互相引用
